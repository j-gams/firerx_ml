...
class model_cascade2_late_a:
    ...
    def make_base_model(self, layer_dims, y_break, unique_dims_sorted, frequency, y_unique, y_frequency, model_params):
        ...
        if self.v != 0:
            print("building cascade2_late_a model")
        ...
        for j in range(len(conv_dims)):
            if conv_dims[j] == 2:
                convs2[j] = tf.keras.layers.Conv2D(filters=16 * sfreq[j], kernel_size=(2, 2), strides=(2, 2),
                                   padding="same")(convs2[j])
        ...
        ### step 10 -- concat
        for j in range(len(conv_dims)):
            if conv_dims[j] == 1 and len(convs2) > 1:
                convs2[j] = tf.keras.layers.Concatenate(axis=3)(convs2[j])
            else:
                print("oh no", j, conv_dims[j], len(convs2))

        ### step 11 -- conv and flatten
        convs2[0] = tf.keras.layers.Conv2D(filters=16 * len(xdims), kernel_size=(1, 1), strides=(1, 1),
